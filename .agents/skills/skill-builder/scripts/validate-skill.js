...
class SkillValidator {
  ...
  async checkStructure() {
    ...
    const optionalDirs = ['scripts', 'templates', 'examples', '_examples', 'tests'];
  ...
  async checkPermissions() {
    ...
    try {
      const configContent = await fs.readFile(configFile, 'utf-8');
      const config = JSON.parse(configContent);
      const perms = config.permissions || config.requirements?.permissions;

      if (perms) {
        // Check for dangerous permissions (flat structure)
