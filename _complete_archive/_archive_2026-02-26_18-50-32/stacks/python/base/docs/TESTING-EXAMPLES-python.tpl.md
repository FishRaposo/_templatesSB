...
# Python Testing Examples - {{PROJECT_NAME}}

**Tier**: {{TIER}} | **Stack**: Python

## 🧪 Testing Strategy Overview

Python testing follows the **testing pyramid**: **Unit Tests > Integration Tests > End-to-End Tests**. Each tier requires different levels of testing rigor with pytest as the primary framework.
...
from {{PROJECT_NAME}}.models.user import User
from {{PROJECT_NAME}}.core.exceptions import AuthenticationError
...
class TestAuthService:
    ...
    @pytest.mark.asyncio
    async def test_authenticate_with_valid_credentials(self):
        # Arrange
        email = "test@example.com"
    ...
    @pytest.mark.asyncio
    async def test_authenticate_with_invalid_password(self):
        # Arrange
        email = "test@example.com"
...
```python
# tests/unit/test_enterprise_order_service.py
import pytest
from unittest.mock import Mock, AsyncMock, patch
from {{PROJECT_NAME}}.services.enterprise_order_service import EnterpriseOrderService
from {{PROJECT_NAME}}.models.order import Order
from {{PROJECT_NAME}}.core.exceptions import BusinessRuleException
...
class TestEnterpriseOrderService:
    ...
    @pytest.mark.asyncio
    async def test_create_order_with_business_rules(self):
        # Arrange
    ...
    @pytest.mark.asyncio
    async def test_create_order_fails_business_validation(self):
        # Arrange
        order_data = CreateOrderRequest(
            user_id="user123",
    ...
    @pytest.mark.asyncio
    async def test_create_order_handles_payment_failure(self):
        # Arrange
        order_data = CreateOrderRequest(
            user_id="user123",
            items=[{"product_id": "prod1", "quantity": 2}],
...
from {{PROJECT_NAME}}.tests.conftest import TestSession

class TestAuthEndpoints:
    def setup_method(self):
        self.client = TestClient(app)
    
    def test_login_with_valid_credentials(self, db_session):
...
from {{PROJECT_NAME}}.repositories.order_repository import OrderRepository
from {{PROJECT_NAME}}.models.order import Order
...
```python
# tests/e2e/test_enterprise_order_flow.py
import pytest
from playwright.async_api import async_playwright

class TestEnterpriseOrderFlow:
    @pytest.mark.asyncio
    async def test_complete_order_with_payment(self):
        async with async_playwright() as p:
            browser = await p.chromium.launch()
            page = await browser.new_page()
...
from sqlalchemy import create_engine
from sqlalchemy.orm import sessionmaker
from {{PROJECT_NAME}}.core.database import Base, get_db
from {{PROJECT_NAME}}.main import app
...
def assert_recent_timestamp(timestamp, minutes=5):
...
def assert_valid_jwt_token(token):
    ...
    import jwt
    from {{PROJECT_NAME}}.core.config import SECRET_KEY, ALGORITHM
...
---
*Python Testing Examples - Use these patterns for comprehensive test coverage*
