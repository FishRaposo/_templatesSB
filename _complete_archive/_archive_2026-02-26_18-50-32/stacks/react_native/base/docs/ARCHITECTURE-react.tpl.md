...
# React Native Architecture Guide - {{PROJECT_NAME}}

**Tier**: {{TIER}} | **Stack**: React Native

## 🏗️ React Native Architecture Overview

React Native applications follow **component-based architecture** with **atomic design principles** and **layered state management**. This ensures maintainability, reusability, and scalability across MVP, CORE, and FULL tiers while leveraging React Native's declarative nature and ecosystem.
...
│   │       ├── Dashboard/
│   │       └── Profile/
│   ├── contexts/                 # React Native Context providers
│   │   ├── AuthContext.tsx
│   │   ├── ThemeContext.tsx
...
import React Native, { useState, useCallback, useMemo } from 'react_native';
import { useSelector, useDispatch } from 'react_native-redux';
...
import { lazy, Suspense } from 'react_native';
...
// Lazy route wrapper
export const LazyRoute = ({ children }: { children: React Native.React NativeNode }) => (
...
import React Native, { useMemo, useCallback } from 'react_native';
import { FixedSizeList as List } from 'react_native-window';
import { useVirtualizer } from '@tanstack/react_native-virtual';
...
interface ColumnConfig<T> {
  ...
  header: string;
  width: number;
  render: (item: T) => React Native.React NativeNode;
}
...
---
*React Native Architecture Guide - Use these patterns for maintainable and scalable React Native applications*
