<!--
File: AUTH-OAUTH-SKELETON.tpl.md
Purpose: Template for auth-oauth implementation
Template Version: 1.0
-->

# AuthOauth Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for auth-oauth service
interface AuthOauthService {
...
- `{{TASK_NAME}}`: Task name (auth-oauth)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/auth-oauth.yaml`:
