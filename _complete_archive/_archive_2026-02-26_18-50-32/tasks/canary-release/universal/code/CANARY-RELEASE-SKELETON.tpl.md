<!--
File: CANARY-RELEASE-SKELETON.tpl.md
Purpose: Template for canary-release implementation
Template Version: 1.0
-->

# CanaryRelease Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for canary-release service
interface CanaryReleaseService {
...
- `{{TASK_NAME}}`: Task name (canary-release)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/canary-release.yaml`:
