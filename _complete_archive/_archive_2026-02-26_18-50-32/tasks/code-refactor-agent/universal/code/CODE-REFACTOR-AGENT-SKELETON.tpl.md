<!--
File: CODE-REFACTOR-AGENT-SKELETON.tpl.md
Purpose: Template for code-refactor-agent implementation
Template Version: 1.0
-->

# CodeRefactorAgent Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for code-refactor-agent service
interface CodeRefactorAgentService {
...
- `{{TASK_NAME}}`: Task name (code-refactor-agent)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/code-refactor-agent.yaml`:
