...
# Code Refactor Agent Overview

## Purpose
Target repo â†’ plan refactor â†’ create diffs â†’ run tests â†’ produce report.

## Features
- **Core Functionality**: Target repo â†’ plan refactor â†’ create diffs â†’ run tests â†’ produce report.
...
const result = await coderefactoragentService.execute(input);
```
...
## Supported Stacks
**Default Stacks**: python, node
**All Supported Stacks**: python, node

## Tier Recommendations
- **basic-refactor**: core
- **enterprise-refactor**: full
...
## Configuration
The task uses configuration from `config/code-refactor-agent.yaml`:
