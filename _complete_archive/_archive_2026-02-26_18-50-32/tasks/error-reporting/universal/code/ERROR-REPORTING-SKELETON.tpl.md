<!--
File: ERROR-REPORTING-SKELETON.tpl.md
Purpose: Template for error-reporting implementation
Template Version: 1.0
-->

# ErrorReporting Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for error-reporting service
...
- `{{TASK_NAME}}`: Task name (error-reporting)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/error-reporting.yaml`:
