...
## Purpose
Centralized logging + external service integration (Sentry-like).

## Features
- **Core Functionality**: Centralized logging + external service integration (Sentry-like).
...
const result = await errorreportingService.execute(input);
```
...
## Configuration
The task uses configuration from `config/error-reporting.yaml`:
