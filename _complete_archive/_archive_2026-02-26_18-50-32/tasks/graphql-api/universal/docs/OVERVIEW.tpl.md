...
## Features
- **Core Functionality**: Schema, resolvers, data loaders, field-level auth.
...
const result = await graphqlapiService.execute(input);
```
...
## Configuration
The task uses configuration from `config/graphql-api.yaml`:
