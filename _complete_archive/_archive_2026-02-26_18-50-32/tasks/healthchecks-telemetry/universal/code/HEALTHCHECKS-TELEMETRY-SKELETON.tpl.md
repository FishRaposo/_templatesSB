<!--
File: HEALTHCHECKS-TELEMETRY-SKELETON.tpl.md
Purpose: Template for healthchecks-telemetry implementation
Template Version: 1.0
-->

# HealthchecksTelemetry Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for healthchecks-telemetry service
interface HealthchecksTelemetryService {
...
- `{{TASK_NAME}}`: Task name (healthchecks-telemetry)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/healthchecks-telemetry.yaml`:
