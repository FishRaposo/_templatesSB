...
# Healthchecks Telemetry Overview

## Purpose
/app/health, metrics endpoints, uptime checks, basic observability wiring.

## Features
- **Core Functionality**: /app/health, metrics endpoints, uptime checks, basic observability wiring.
...
const result = await healthcheckstelemetryService.execute(input);
```

### Advanced Configuration
```yaml
# Advanced configuration example
healthchecks-telemetry:
...
## Supported Stacks
**Default Stacks**: go, node, python
**All Supported Stacks**: go, node, python
...
## Configuration
The task uses configuration from `config/healthchecks-telemetry.yaml`:
