<!--
File: JOB-QUEUE-SKELETON.tpl.md
Purpose: Template for job-queue implementation
Template Version: 1.0
-->

# JobQueue Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for job-queue service
...
- `{{TASK_NAME}}`: Task name (job-queue)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/job-queue.yaml`:
