...
## Purpose
Given stack + tier + tasks â†’ assemble everything (you already kind of have this).

## Features
- **Core Functionality**: Given stack + tier + tasks â†’ assemble everything (you already kind of have this).
...
const result = await projectbootstrapService.execute(input);
```
...
## Configuration
The task uses configuration from `config/project-bootstrap.yaml`:
