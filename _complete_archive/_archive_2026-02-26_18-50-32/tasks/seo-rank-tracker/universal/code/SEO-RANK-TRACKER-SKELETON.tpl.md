<!--
File: SEO-RANK-TRACKER-SKELETON.tpl.md
Purpose: Template for seo-rank-tracker implementation
Template Version: 1.0
-->

# SeoRankTracker Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for seo-rank-tracker service
interface SeoRankTrackerService {
...
- `{{TASK_NAME}}`: Task name (seo-rank-tracker)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/seo-rank-tracker.yaml`:
