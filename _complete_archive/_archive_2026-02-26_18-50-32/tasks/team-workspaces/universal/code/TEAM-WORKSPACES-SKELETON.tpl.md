<!--
File: TEAM-WORKSPACES-SKELETON.tpl.md
Purpose: Template for team-workspaces implementation
Template Version: 1.0
-->

# TeamWorkspaces Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for team-workspaces service
interface TeamWorkspacesService {
...
- `{{TASK_NAME}}`: Task name (team-workspaces)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/team-workspaces.yaml`:
