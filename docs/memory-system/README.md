...
├── memory-system-setup/          # Memory System Setup skill (for AI agents)
│   ├── SKILL.md                  # Step-by-step setup instructions (name: memory-system-setup)
│   ├── config.json               # Skill triggers and configuration
│   ├── README.md                 # Skill overview
│   ├── agents-integration-snippet.md
│   ├── event-format-and-types.md
...
**Option A — Script (when memory-system is in the project, e.g. submodule or copied):**

```bash
# From project root
python memory-system/scripts/initialize-memory.py my-project mvp   # MVP
python memory-system/scripts/initialize-memory.py my-project core  # Core
python memory-system/scripts/initialize-memory.py my-project full # Full
```
...
**MVP**:
```bash
cp memory-system/templates/changelog.md CHANGELOG.md
```
...
cp memory-system/templates/changelog.md CHANGELOG.md
mkdir -p .memory
cp memory-system/templates/context.md .memory/context.md
```
...
cp memory-system/templates/changelog.md CHANGELOG.md
mkdir -p .memory
cp memory-system/templates/graph.md .memory/graph.md
cp memory-system/templates/context.md .memory/context.md
```
...
When auditing a deployed memory system, run from project root:

```bash
python memory-system/scripts/validate-memory.py   # or --project-root PATH
```
...
- `memory-system-setup/SKILL.md` — **Memory System Setup** skill for AI agents (name: memory-system-setup)
