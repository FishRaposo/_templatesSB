...
The event log is the source of truth. Create `CHANGELOG.md` at the project root:

```markdown
# Changelog

All notable changes to this project will be documented in this file.
This is the **single source of truth** for project history — the Layer 1 Event Log.

## [Unreleased]

### Added
- Project initialized with memory system (evt-001)
...
### evt-001 | YYYY-MM-DD HH:MM | human | milestone

**Scope**: project
**Summary**: Project initialized with memory system
...
The file has two sections: a **categorized summary** at top (Keep a Changelog format for quick scanning) and the **structured Event Log** below (source of truth for graph and context).
...
The knowledge graph tracks entities and relationships. Initialize with the project's existing components:
...
The narrative provides current trajectory. Initialize from the project's current state:

```markdown
# Current Context

<!-- LAYER 3: Derived projection. Ephemeral. Regenerate from changelog.md + graph.md when stale. -->
<!-- Last generated: YYYY-MM-DD HH:MM | Event horizon: evt-NNN -->

## Active Mission

(One paragraph describing the project's current goal)
...
(Recent decision events that define boundaries)
...
(Last 20 events or 48 hours, newest first)

## Key Dependencies

(depends_on edges for active components)
...
- See `../MEMORY-SYSTEM-PROTOCOL.md` for the full protocol specification (ACID guarantees, multi-agent coordination, handoff protocol, tier scaling)
- See `./_examples/worked-example.md` for 8 events flowing through all 4 layers
- See `../_complete_archive/PROJECT-MEMORY-SYSTEM-REFERENCE.md` for the archive's original memory system
