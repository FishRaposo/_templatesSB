...
3. **Edit**: `name` (gerund, lowercase, hyphens, max 64 chars), `description` ("Use this skill when..." + triggers, max 1024 chars), six required sections, real examples. Keep SKILL.md under 500 lines.
4. **Validate** (from this repo): `node scripts/validate-skill.js path/to/skill`.

**Packs:** `../skill-packs/HOW_TO_CREATE_SKILL_PACKS.md`  
**Walkthrough:** `creating-skills-from-scratch.md`  
**Examples:** `_examples/complete-skill-cli-focused.md`, `_examples/complete-skill-conceptual.md`  
...
- **Required sections:** Core Approach, Step-by-Step Instructions, Best Practices, Validation Checklist, Troubleshooting, Related Skills.
- **config.json:** `agent_support`, `triggers` (keywords, patterns), `requirements` with `"tools": []`.
- **Structure:** `skill-name/SKILL.md`, optional `README.md`, `config.json` (use `templates/config-template.json`; set `"tools": []`), `examples/` or `_examples/` (this repo: `_examples/`). Use intention-revealing file names.
