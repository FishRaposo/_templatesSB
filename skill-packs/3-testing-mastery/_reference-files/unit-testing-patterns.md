<!-- Generated from task-outputs/task-01-unit-testing.md -->

# Unit Testing with Test Doubles

A comprehensive guide to writing unit tests with mocks, stubs, and fakes for service dependencies. This reference demonstrates testing patterns for an OrderService with PaymentGateway, InventoryService, and EmailService dependencies.
...
- Handling edge cases: empty cart, invalid discounts, out-of-stock items
- Multi-language examples (JavaScript, Python, Go)
...
1. **Test doubles enable complete isolation** — Each test controls all dependencies
2. **Arrange-Act-Assert structure** — Clear separation makes tests readable
3. **Parameterized tests** — Reduced duplication for similar test cases
4. **Mock verification** — Ensuring critical side effects occur

## Key Patterns

| Pattern | Usage |
|---------|-------|
| Mock at boundaries | Only mock external services (payment, email, inventory) |
| Test behavior | Tests verify outcomes, not implementation details |
| Fast tests | All tests run in < 10ms without real network/database |
| Clear failure messages | Descriptive test names explain expected behavior |
