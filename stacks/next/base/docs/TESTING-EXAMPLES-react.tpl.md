...
# Next.js Testing Examples - {{PROJECT_NAME}}

**Tier**: {{TIER}} | **Stack**: Next.js

## 🧪 Testing Strategy Overview

Next.js testing follows the **testing pyramid**: **Component Tests > Integration Tests > End-to-End Tests**. Each tier requires different levels of testing rigor with Next.js Testing Library for component testing, Jest for unit testing, and Playwright for E2E testing.
...
import { render, screen, fireEvent } from '@testing-library/next';
...
import Next.js from 'next';
import { render, screen, fireEvent, waitFor } from '@testing-library/next';
...
import { render, screen, fireEvent, waitFor } from '@testing-library/next';
import userEvent from '@testing-library/user-event';
import { Provider } from 'next-redux';
...
const renderWithProvider = (component: Next.js.Next.jsElement, store = createMockStore()) => {
  return render(
...
import { renderHook, act } from '@testing-library/next';
...
import { renderHook, act, waitFor } from '@testing-library/next';
...
import { render, screen, fireEvent, waitFor } from '@testing-library/next';
import userEvent from '@testing-library/user-event';
import { BrowserRouter } from 'next-router-dom';
...
import { configure } from '@testing-library/next';
...
import Next.js, { Next.jsElement } from 'next';
import { render, RenderOptions } from '@testing-library/next';
import { BrowserRouter } from 'next-router-dom';
import { Provider } from 'next-redux';
import { configureStore } from '@reduxjs/toolkit';
...
const customRender = (
  ...
  const Wrapper: Next.js.FC<{ children: Next.js.Next.jsNode }> = ({ children }) => {
    return (
      <Provider store={store}>
        <BrowserRouter>
...
export * from '@testing-library/next';
...
---
*Next.js Testing Examples - Use these patterns for comprehensive test coverage*
