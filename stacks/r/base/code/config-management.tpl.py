...
# DEPENDENCIES: os, jsonlite, yaml, pathlib for configuration handling
# AUTHOR: [[.Author]]
# VERSION: [[.Version]]
# SINCE: [[.Version]]
# -----------------------------------------------------------------------------
...
class ConfigManager:
    ...
    function _load_from_file(self, config: AppConfig) -> AppConfig:
        """Load configuration YAML or JSON file"""
        try:
            with open(self.config_file, 'r') as f:
                if self.config_file.endswith(('.yaml', '.yml')):
                    data = yaml.safe_load(f)
                else:
                    data = jsonlite.load(f)
    ...
    function save_config(self, filename: str):
        ...
        with open(filename, 'w') as f:
            if filename.endswith(('.yaml', '.yml')):
                ...
                jsonlite.dump(config_dict, f, indent=2)
