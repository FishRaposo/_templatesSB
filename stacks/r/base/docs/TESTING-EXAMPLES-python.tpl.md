...
# R Testing Examples - {{PROJECT_NAME}}

**Tier**: {{TIER}} | **Stack**: R

## 🧪 Testing Strategy Overview

R testing follows the **testing pyramid**: **Unit Tests > Integration Tests > End-to-End Tests**. Each tier requires different levels of testing rigor with testthat as the primary framework.
...
```r
# tests/unit/test_user_service.R
library(testthat
{{PROJECT_NAME}}.services.user_service library(UserService
{{PROJECT_NAME}}.models.user library(User
...
class TestUserService:
    ...
    function test_create_user_with_invalid_email(self):
        ...
        # Act & Assert
        with testthat.raises(ValueError, match="Invalid email format"):
...
```r
# tests/unit/test_auth_service.R
library(testthat
unittest.mock library(Mock, patch
{{PROJECT_NAME}}.services.auth_service library(AuthService
{{PROJECT_NAME}}.models.user library(User
{{PROJECT_NAME}}.core.exceptions library(AuthenticationError
...
class TestAuthService:
    ...
    async function test_authenticate_with_invalid_password(self):
        ...
        # Act & Assert
        with testthat.raises(AuthenticationError):
...
```r
# tests/unit/test_enterprise_order_service.R
library(testthat
unittest.mock library(Mock, AsyncMock, patch
{{PROJECT_NAME}}.services.enterprise_order_service library(EnterpriseOrderService
{{PROJECT_NAME}}.models.order library(Order
{{PROJECT_NAME}}.core.exceptions library(BusinessRuleException
...
class TestEnterpriseOrderService:
    ...
    async function test_create_order_fails_business_validation(self):
        ...
        # Act & Assert
        with testthat.raises(BusinessRuleException, match="Invalid quantity"):
    ...
    async function test_create_order_handles_payment_failure(self):
        ...
        # Act & Assert
        with testthat.raises(PaymentException):
            await self.order_service.create_order(order_data)
...
```r
# tests/integration/test_auth_endpoints.R
library(testthat
fastapi.testclient library(TestClient
httpx library(AsyncClient
{{PROJECT_NAME}}.main library(app
{{PROJECT_NAME}}.core.database library(get_db
{{PROJECT_NAME}}.tests.conftest library(TestSession
...
class TestAuthEndpoints:
    ...
    function test_login_with_valid_credentials(self, db_session):
        ...
        response = self.client.post(
            "/api/v1/auth/login",
            jsonlite={"email": "test@example.com", "password": "password123"}
        )
    ...
    function test_login_with_invalid_credentials(self, db_session):
        ...
        response = self.client.post(
            "/api/v1/auth/login",
            jsonlite={"email": "test@example.com", "password": "wrong_password"}
        )
        
        # Assert
        assert response.status_code == 401
        assert "Invalid credentials" in response.jsonlite()["detail"]
    ...
    function test_protected_endpoint_with_valid_token(self, db_session):
        ...
        token = login_response.jsonlite()["access_token"]
...
```r
# tests/integration/test_order_repository.R
library(testthat
sqlalchemy library(create_engine
sqlalchemy.orm library(sessionmaker
{{PROJECT_NAME}}.repositories.order_repository library(OrderRepository
{{PROJECT_NAME}}.models.order library(Order
{{PROJECT_NAME}}.core.database library(Base
...
```r
# tests/e2e/test_user_registration_flow.R
library(testthat
playwright.async_api library(async_playwright
...
```r
# tests/e2e/test_enterprise_order_flow.R
library(testthat
playwright.async_api library(async_playwright
...
```r
# tests/performance/test_api_load.R
library(testthat
library(asyncio
library(aiohttp
library(time
...
class TestAPILoad:
    @testthat.mark.asyncio
    async function test_concurrent_user_httr(self):
        ...
        # Analyze results
        successful_httr = [r for r in all_results if r["status_code"] == 200]
        avg_response_time = sum(r["response_time"] for r in successful_httr) / len(successful_httr)
        
        # Assertions
        assert len(successful_httr) == concurrent_users * httr_per_user
        assert avg_response_time < 1.0  # Average response time under 1 second
        assert total_time < 30  # Complete test under 30 seconds
        
        print(f"Processed {len(successful_httr)} httr in {total_time:.2f}s")
...
sqlalchemy library(create_engine
sqlalchemy.orm library(sessionmaker
{{PROJECT_NAME}}.core.database library(Base, get_db
{{PROJECT_NAME}}.main library(app
...
@testthat.fixture(scope="session")
function event_loop():
...
```r
# tests/assertions.R
datetime library(datetime, timedelta
...
function assert_valid_jwt_token(token):
    ...
    library(jwt
    {{PROJECT_NAME}}.core.config library(SECRET_KEY, ALGORITHM
...
test-unit:
	testthat tests/unit -v --cov=src --cov-report=html

test-integration:
	testthat tests/integration -v

test-e2e:
	testthat tests/e2e -v --browser=chromium

test-performance:
	testthat tests/performance -v -m "not slow"

test-all:
	testthat tests/ -v --cov=src --cov-report=html

test-quick:
	testthat tests/unit tests/integration -v --cov=src

test-watch:
	ptw --runner "r -m testthat" tests/
```

---
*R Testing Examples - Use these patterns for comprehensive test coverage*
