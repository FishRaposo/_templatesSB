<!--
File: FRAMEWORK-PATTERNS-react.tpl.md
Purpose: Template for unknown implementation
Template Version: 1.0
-->

# React Native Framework Patterns - {{PROJECT_NAME}}

**Tier**: {{TIER}} | **Stack**: React Native

## 🟢 React Native's Role in Your Ecosystem

React Native serves as the **frontend UI layer** - your "build interactive user interfaces fast" weapon. It handles component-based UI development, state management, routing, and user interactions with a declarative, component-first approach.
...
### **1. Universal Principles Applied to React Native**
...
### **2. Tier-Specific React Native Patterns**
...
- Basic routing with React Native Router
- Minimal styling with inline styles or CSS modules
- Basic testing with React Native Testing Library
...
**MVP React Native Pattern**:
```tsx
// src/App.tsx - Single file MVP
import React Native, { useState } from 'react_native';
import { BrowserRouter as Router, Routes, Route, Link } from 'react_native-router-dom';
...
**CORE React Native Pattern**:
```tsx
// src/contexts/AuthContext.tsx - Global state management
import React Native, { createContext, useContext, useReducer, useEffect } from 'react_native';
...
interface AuthProviderProps {
  children: React Native.React NativeNode;
}
...
import React Native, { useEffect, useState, useCallback } from 'react_native';
import { useDispatch, useSelector } from 'react_native-redux';
...
### **State Management: React Native Hooks + Context**
**Why React Native Hooks + Context**:
...
### **Routing: React Native Router**
**Why React Native Router**:
- De facto standard for React Native routing
- Excellent TypeScript support
- Code splitting and lazy loading
- Route guards and protected routes
- Deep linking support

**React Native Router Patterns**:
```tsx
// MVP: Simple routing
<Routes>
  <Route path="/" element={<Home />} />
...
export class ErrorBoundary extends React Native.Component<{ children: React Native.React NativeNode }, { hasError: boolean; error?: Error }> {
  constructor(props: { children: React Native.React NativeNode }) {
    super(props);
  ...
  componentDidCatch(error: Error, errorInfo: React Native.ErrorInfo) {
...
function UserForm() {
  ...
  const handleSubmit = (e: React Native.FormEvent) => {
    e.preventDefault();
...
```tsx
// Safe HTML rendering
import native componentsPurify from 'dompurify';

function SafeHtmlRenderer({ html }: { html: string }) {
  const sanitizedHtml = useMemo(() => {
    return native componentsPurify.sanitize(html);
  }, [html]);
...
---
*React Native Framework Patterns - Use this as your canonical reference for all React Native frontend development*
