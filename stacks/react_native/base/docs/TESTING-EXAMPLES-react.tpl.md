...
React Native testing follows the **testing pyramid**: **Component Tests > Integration Tests > End-to-End Tests**. Each tier requires different levels of testing rigor with React Native Testing Library for component testing, Jest for unit testing, and Playwright for E2E testing.
...
import { render, screen, fireEvent } from '@testing-library/react_native';
...
import React Native from 'react_native';
import { render, screen, fireEvent, waitFor } from '@testing-library/react_native';
...
import { render, screen, fireEvent, waitFor } from '@testing-library/react_native';
import userEvent from '@testing-library/user-event';
import { Provider } from 'react_native-redux';
...
const renderWithProvider = (component: React Native.React NativeElement, store = createMockStore()) => {
  return render(
...
import { renderHook, act } from '@testing-library/react_native';
...
import { renderHook, act, waitFor } from '@testing-library/react_native';
...
import { render, screen, fireEvent, waitFor } from '@testing-library/react_native';
import userEvent from '@testing-library/user-event';
import { BrowserRouter } from 'react_native-router-dom';
...
import { configure } from '@testing-library/react_native';
...
import React Native, { React NativeElement } from 'react_native';
import { render, RenderOptions } from '@testing-library/react_native';
import { BrowserRouter } from 'react_native-router-dom';
import { Provider } from 'react_native-redux';
import { configureStore } from '@reduxjs/toolkit';
...
const customRender = (
  ...
  const Wrapper: React Native.FC<{ children: React Native.React NativeNode }> = ({ children }) => {
    return (
      <Provider store={store}>
        <BrowserRouter>
...
export * from '@testing-library/react_native';
...
---
*React Native Testing Examples - Use these patterns for comprehensive test coverage*
