<!--
File: AB-TEST-ANALYSIS-SKELETON.tpl.md
Purpose: Template for ab-test-analysis implementation
Template Version: 1.0
-->

# AbTestAnalysis Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for ab-test-analysis service
interface AbTestAnalysisService {
...
- `{{TASK_NAME}}`: Task name (ab-test-analysis)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/ab-test-analysis.yaml`:
