...
# Ab Test Analysis Overview

## Purpose
Analyze A/B test results, significance, uplift, power.

## Features
- **Core Functionality**: Analyze A/B test results, significance, uplift, power.
...
const result = await abtestanalysisService.execute(input);
```
...
## Configuration
The task uses configuration from `config/ab-test-analysis.yaml`:
