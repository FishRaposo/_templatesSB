...
## Purpose
Superuser UI for managing users, config, logs, feature flags.

## Features
- **Core Functionality**: Superuser UI for managing users, config, logs, feature flags.
...
const result = await adminpanelService.execute(input);
```
...
**All Supported Stacks**: nextjs, react, node, go

## Tier Recommendations
- **basic-admin**: core
- **enterprise-admin**: full
...
## Configuration
The task uses configuration from `config/admin-panel.yaml`:
