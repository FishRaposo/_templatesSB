...
## Purpose
Multi-step agent orchestration: planner, executor, verifier.

## Features
- **Core Functionality**: Multi-step agent orchestration: planner, executor, verifier.
...
const result = await agenticworkflowService.execute(input);
```
...
## Configuration
The task uses configuration from `config/agentic-workflow.yaml`:
