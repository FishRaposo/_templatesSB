...
# Analytics Event Pipeline Overview

## Purpose
Collect events, batch/stream, aggregate, expose to dashboard.

## Features
- **Core Functionality**: Collect events, batch/stream, aggregate, expose to dashboard.
...
const result = await analyticseventpipelineService.execute(input);
```
...
**All Supported Stacks**: node, go, sql, r

## Tier Recommendations
- **basic-analytics**: core
- **enterprise-analytics**: full
...
## Configuration
The task uses configuration from `config/analytics-event-pipeline.yaml`:
