...
## Features
- **Core Functionality**: Who did what, when, on which object, with replayable logs.
...
const result = await auditloggingService.execute(input);
```
...
## Supported Stacks
**Default Stacks**: node, go, python
**All Supported Stacks**: node, go, python, sql

## Tier Recommendations
- **basic-audit**: core
- **enterprise-audit**: full
...
## Configuration
The task uses configuration from `config/audit-logging.yaml`:
