<!--
File: AUTH-BASIC-SKELETON.tpl.md
Purpose: Template for auth-basic implementation
Template Version: 1.0
-->

# AuthBasic Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for auth-basic service
interface AuthBasicService {
...
- `{{TASK_NAME}}`: Task name (auth-basic)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/auth-basic.yaml`:
