...
## Purpose
Email/password, sessions/JWT, password reset.

## Features
- **Core Functionality**: Email/password, sessions/JWT, password reset.
...
const result = await authbasicService.execute(input);
```
...
**All Supported Stacks**: node, python, nextjs
...
## Configuration
The task uses configuration from `config/auth-basic.yaml`:
