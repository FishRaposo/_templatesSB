...
## Purpose
Google/GitHub/Twitter/etc. login flows, token storage, linking accounts.

## Features
- **Core Functionality**: Google/GitHub/Twitter/etc. login flows, token storage, linking accounts.
...
const result = await authoauthService.execute(input);
```
...
## Supported Stacks
**Default Stacks**: node, nextjs
**All Supported Stacks**: node, nextjs, flutter, react

## Tier Recommendations
- **oauth-integration**: core
- **enterprise-oauth**: full
...
## Configuration
The task uses configuration from `config/auth-oauth.yaml`:
