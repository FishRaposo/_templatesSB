<!--
File: BILLING-STRIPE-SKELETON.tpl.md
Purpose: Template for billing-stripe implementation
Template Version: 1.0
-->

# BillingStripe Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for billing-stripe service
interface BillingStripeService {
...
- `{{TASK_NAME}}`: Task name (billing-stripe)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/billing-stripe.yaml`:
