<!--
File: CI-TEMPLATE-SKELETON.tpl.md
Purpose: Template for ci-template implementation
Template Version: 1.0
-->

# CiTemplate Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for ci-template service
interface CiTemplateService {
...
- `{{TASK_NAME}}`: Task name (ci-template)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/ci-template.yaml`:
