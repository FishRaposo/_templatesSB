...
## Purpose
GitHub Actions / GitLab CI / etc: lint â†’ test â†’ build â†’ deploy.

## Features
- **Core Functionality**: GitHub Actions / GitLab CI / etc: lint â†’ test â†’ build â†’ deploy.
...
const result = await citemplateService.execute(input);
```
...
## Supported Stacks
**Default Stacks**: agnostic
**All Supported Stacks**: agnostic
...
## Configuration
The task uses configuration from `config/ci-template.yaml`:
