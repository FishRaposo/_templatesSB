<!--
File: CONFIG-MANAGEMENT-SKELETON.tpl.md
Purpose: Template for config-management implementation
Template Version: 1.0
-->

# ConfigManagement Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for config-management service
interface ConfigManagementService {
...
- `{{TASK_NAME}}`: Task name (config-management)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/config-management.yaml`:
