...
## Purpose
12-factor style config: env vars, secrets, per-env files.

## Features
- **Core Functionality**: 12-factor style config: env vars, secrets, per-env files.
...
const result = await configmanagementService.execute(input);
```
...
## Supported Stacks
**Default Stacks**: all
**All Supported Stacks**: python, go, node, react, nextjs, flutter, sql, r

## Tier Recommendations
- **basic-config**: mvp
- **production-config**: core
- **enterprise-config**: full
...
## Configuration
The task uses configuration from `config/config-management.yaml`:
