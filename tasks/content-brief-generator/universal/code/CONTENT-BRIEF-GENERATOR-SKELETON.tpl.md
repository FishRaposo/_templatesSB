...
## Interface
```{{EXTENSION}}
// Universal interface for content-brief-generator service
interface ContentBriefGeneratorService {
...
- `{{TASK_NAME}}`: Task name (content-brief-generator)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/content-brief-generator.yaml`:
