...
# Content Brief Generator Overview

## Purpose
Given keyword sets â†’ outline + headings + questions + internal link suggestions.

## Features
- **Core Functionality**: Given keyword sets â†’ outline + headings + questions + internal link suggestions.
...
const result = await contentbriefgeneratorService.execute(input);
```
...
**All Supported Stacks**: python, nextjs

## Tier Recommendations
- **basic-generation**: mvp
...
## Configuration
The task uses configuration from `config/content-brief-generator.yaml`:
