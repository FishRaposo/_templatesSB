...
# Crud Module Overview

## Purpose
Generic CRUD stack: model definition â†’ API â†’ forms â†’ tables â†’ filters.

## Features
- **Core Functionality**: Generic CRUD stack: model definition â†’ API â†’ forms â†’ tables â†’ filters.
...
const result = await crudmoduleService.execute(input);
```
...
## Supported Stacks
**Default Stacks**: node, python
**All Supported Stacks**: node, python, go, react, nextjs, flutter

## Tier Recommendations
- **basic-crud**: mvp
- **production-crud**: core
- **enterprise-crud**: full
...
## Configuration
The task uses configuration from `config/crud-module.yaml`:
