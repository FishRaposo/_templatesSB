<!--
File: DATA-EXPLORATION-REPORT-SKELETON.tpl.md
Purpose: Template for data-exploration-report implementation
Template Version: 1.0
-->

# DataExplorationReport Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for data-exploration-report service
interface DataExplorationReportService {
...
- `{{TASK_NAME}}`: Task name (data-exploration-report)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/data-exploration-report.yaml`:
