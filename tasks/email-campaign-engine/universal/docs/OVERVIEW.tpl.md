...
# Email Campaign Engine Overview

## Purpose
Segment lists, define campaigns, send, track opens/clicks, basic automations.

## Features
- **Core Functionality**: Segment lists, define campaigns, send, track opens/clicks, basic automations.
...
const result = await emailcampaignengineService.execute(input);
```
...
**All Supported Stacks**: node, python, react, nextjs, sql

## Tier Recommendations
- **basic-campaigns**: core
- **enterprise-campaigns**: full
...
## Configuration
The task uses configuration from `config/email-campaign-engine.yaml`:
