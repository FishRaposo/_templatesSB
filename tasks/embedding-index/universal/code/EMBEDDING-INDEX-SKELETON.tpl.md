<!--
File: EMBEDDING-INDEX-SKELETON.tpl.md
Purpose: Template for embedding-index implementation
Template Version: 1.0
-->

# EmbeddingIndex Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for embedding-index service
interface EmbeddingIndexService {
...
- `{{TASK_NAME}}`: Task name (embedding-index)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/embedding-index.yaml`:
