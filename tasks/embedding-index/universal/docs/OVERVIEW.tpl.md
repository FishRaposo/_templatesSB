...
# Embedding Index Overview

## Purpose
Create and query vector index for semantic search/retrieval.

## Features
- **Core Functionality**: Create and query vector index for semantic search/retrieval.
...
const result = await embeddingindexService.execute(input);
```
...
**All Supported Stacks**: python, node, sql

## Tier Recommendations
- **basic-embeddings**: core
- **enterprise-embeddings**: full
...
## Configuration
The task uses configuration from `config/embedding-index.yaml`:
