...
## Interface
```{{EXTENSION}}
// Universal interface for etl-pipeline service
interface EtlPipelineService {
...
- `{{TASK_NAME}}`: Task name (etl-pipeline)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/etl-pipeline.yaml`:
