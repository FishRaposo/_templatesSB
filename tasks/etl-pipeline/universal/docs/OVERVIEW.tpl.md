...
## Purpose
Extract â†’ Transform â†’ Load between APIs/DBs/data lakes.

## Features
- **Core Functionality**: Extract â†’ Transform â†’ Load between APIs/DBs/data lakes.
...
const result = await etlpipelineService.execute(input);
```
...
## Supported Stacks
**Default Stacks**: python, sql
**All Supported Stacks**: python, sql, go
...
## Configuration
The task uses configuration from `config/etl-pipeline.yaml`:
