...
## Purpose
Toggle features by user, cohort, or environment with UI + SDK.

## Features
- **Core Functionality**: Toggle features by user, cohort, or environment with UI + SDK.
...
const result = await featureflagsService.execute(input);
```
...
**All Supported Stacks**: node, go, react, nextjs

## Tier Recommendations
- **basic-flags**: core
- **enterprise-flags**: full
...
## Configuration
The task uses configuration from `config/feature-flags.yaml`:
