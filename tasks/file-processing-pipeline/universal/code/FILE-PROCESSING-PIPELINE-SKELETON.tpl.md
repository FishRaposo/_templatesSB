<!--
File: FILE-PROCESSING-PIPELINE-SKELETON.tpl.md
Purpose: Template for file-processing-pipeline implementation
Template Version: 1.0
-->

# FileProcessingPipeline Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for file-processing-pipeline service
interface FileProcessingPipelineService {
...
- `{{TASK_NAME}}`: Task name (file-processing-pipeline)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/file-processing-pipeline.yaml`:
