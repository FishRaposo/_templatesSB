<!--
File: FORECASTING-ENGINE-SKELETON.tpl.md
Purpose: Template for forecasting-engine implementation
Template Version: 1.0
-->

# ForecastingEngine Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for forecasting-engine service
interface ForecastingEngineService {
...
- `{{TASK_NAME}}`: Task name (forecasting-engine)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/forecasting-engine.yaml`:
