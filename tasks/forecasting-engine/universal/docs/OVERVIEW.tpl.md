...
## Purpose
Time-series forecasting: trends, seasonality, prediction intervals.

## Features
- **Core Functionality**: Time-series forecasting: trends, seasonality, prediction intervals.
...
const result = await forecastingengineService.execute(input);
```
...
- **basic-forecasting**: core
- **enterprise-forecasting**: full
...
## Configuration
The task uses configuration from `config/forecasting-engine.yaml`:
