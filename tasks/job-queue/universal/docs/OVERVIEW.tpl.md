...
## Purpose
Generic job queue with workers, retries, dead-letter queues, metrics.

## Features
- **Core Functionality**: Generic job queue with workers, retries, dead-letter queues, metrics.
...
const result = await jobqueueService.execute(input);
```
...
## Configuration
The task uses configuration from `config/job-queue.yaml`:
