<!--
File: LINK-MONITORING-SKELETON.tpl.md
Purpose: Template for link-monitoring implementation
Template Version: 1.0
-->

# LinkMonitoring Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for link-monitoring service
interface LinkMonitoringService {
...
- `{{TASK_NAME}}`: Task name (link-monitoring)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/link-monitoring.yaml`:
