...
## Purpose
Track backlinks, broken links, redirects, domain health.

## Features
- **Core Functionality**: Track backlinks, broken links, redirects, domain health.
...
const result = await linkmonitoringService.execute(input);
```
...
## Supported Stacks
**Default Stacks**: go, python
**All Supported Stacks**: go, python, sql, nextjs

## Tier Recommendations
- **basic-monitoring**: core
- **enterprise-monitoring**: full
...
## Configuration
The task uses configuration from `config/link-monitoring.yaml`:
