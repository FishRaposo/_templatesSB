...
# Llm Prompt Router Overview

## Purpose
Given task â†’ choose model, prompt template, tools, temp, etc.

## Features
- **Core Functionality**: Given task â†’ choose model, prompt template, tools, temp, etc.
...
const result = await llmpromptrouterService.execute(input);
```
...
## Configuration
The task uses configuration from `config/llm-prompt-router.yaml`:
