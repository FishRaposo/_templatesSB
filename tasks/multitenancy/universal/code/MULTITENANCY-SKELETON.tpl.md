<!--
File: MULTITENANCY-SKELETON.tpl.md
Purpose: Template for multitenancy implementation
Template Version: 1.0
-->

# Multitenancy Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for multitenancy service
...
- `{{TASK_NAME}}`: Task name (multitenancy)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/multitenancy.yaml`:
