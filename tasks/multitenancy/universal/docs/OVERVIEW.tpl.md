...
## Purpose
Workspace/tenant isolation: per-tenant data, routing, billing hooks.

## Features
- **Core Functionality**: Workspace/tenant isolation: per-tenant data, routing, billing hooks.
...
const result = await multitenancyService.execute(input);
```
...
**All Supported Stacks**: node, go, sql

## Tier Recommendations
- **basic-multitenancy**: full
...
## Configuration
The task uses configuration from `config/multitenancy.yaml`:
