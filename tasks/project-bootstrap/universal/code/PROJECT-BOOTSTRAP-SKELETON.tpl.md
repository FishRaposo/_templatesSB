...
## Interface
```{{EXTENSION}}
// Universal interface for project-bootstrap service
interface ProjectBootstrapService {
...
- `{{TASK_NAME}}`: Task name (project-bootstrap)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/project-bootstrap.yaml`:
