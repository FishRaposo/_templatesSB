...
## Interface
```{{EXTENSION}}
// Universal interface for public-api-gateway service
interface PublicApiGatewayService {
...
- `{{TASK_NAME}}`: Task name (public-api-gateway)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/public-api-gateway.yaml`:
