...
# Public Api Gateway Overview

## Purpose
Wrap internal services with external-facing secure API: keys, quotas, logging.

## Features
- **Core Functionality**: Wrap internal services with external-facing secure API: keys, quotas, logging.
...
const result = await publicapigatewayService.execute(input);
```
...
## Supported Stacks
**Default Stacks**: node, go
**All Supported Stacks**: node, go

## Tier Recommendations
- **basic-gateway**: core
- **enterprise-gateway**: full
...
## Configuration
The task uses configuration from `config/public-api-gateway.yaml`:
