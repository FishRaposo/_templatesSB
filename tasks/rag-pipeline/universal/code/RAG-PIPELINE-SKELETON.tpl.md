<!--
File: RAG-PIPELINE-SKELETON.tpl.md
Purpose: Template for rag-pipeline implementation
Template Version: 1.0
-->

# RagPipeline Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for rag-pipeline service
...
- `{{TASK_NAME}}`: Task name (rag-pipeline)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/rag-pipeline.yaml`:
