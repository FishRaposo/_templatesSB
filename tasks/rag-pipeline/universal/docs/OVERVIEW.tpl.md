...
## Purpose
Ingest â†’ chunk â†’ embed â†’ store â†’ query â†’ answer generation.

## Features
- **Core Functionality**: Ingest â†’ chunk â†’ embed â†’ store â†’ query â†’ answer generation.
...
const result = await ragpipelineService.execute(input);
```
...
**All Supported Stacks**: python, sql
...
## Configuration
The task uses configuration from `config/rag-pipeline.yaml`:
