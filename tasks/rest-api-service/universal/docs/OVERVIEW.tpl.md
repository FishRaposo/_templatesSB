...
## Features
- **Core Functionality**: Standard CRUD API with auth, validation, pagination, rate limits.
...
const result = await restapiserviceService.execute(input);
```
...
**All Supported Stacks**: node, python, go, nextjs
...
## Configuration
The task uses configuration from `config/rest-api-service.yaml`:
