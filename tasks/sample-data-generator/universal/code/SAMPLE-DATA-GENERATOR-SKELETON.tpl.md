<!--
File: SAMPLE-DATA-GENERATOR-SKELETON.tpl.md
Purpose: Template for sample-data-generator implementation
Template Version: 1.0
-->

# SampleDataGenerator Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for sample-data-generator service
interface SampleDataGeneratorService {
...
- `{{TASK_NAME}}`: Task name (sample-data-generator)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/sample-data-generator.yaml`:
