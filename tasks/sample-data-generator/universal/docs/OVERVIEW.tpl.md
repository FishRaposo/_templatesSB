...
## Purpose
Fixtures + realistic fake data for dev/staging environments.

## Features
- **Core Functionality**: Fixtures + realistic fake data for dev/staging environments.
...
const result = await sampledatageneratorService.execute(input);
```
...
## Configuration
The task uses configuration from `config/sample-data-generator.yaml`:
