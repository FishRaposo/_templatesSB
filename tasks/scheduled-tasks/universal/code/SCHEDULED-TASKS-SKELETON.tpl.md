<!--
File: SCHEDULED-TASKS-SKELETON.tpl.md
Purpose: Template for scheduled-tasks implementation
Template Version: 1.0
-->

# ScheduledTasks Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for scheduled-tasks service
...
- `{{TASK_NAME}}`: Task name (scheduled-tasks)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/scheduled-tasks.yaml`:
