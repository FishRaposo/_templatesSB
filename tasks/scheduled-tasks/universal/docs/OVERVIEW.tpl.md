...
## Purpose
Cron-like jobs: cleanup, syncs, data refresh, email digests.

## Features
- **Core Functionality**: Cron-like jobs: cleanup, syncs, data refresh, email digests.
...
const result = await scheduledtasksService.execute(input);
```
...
**All Supported Stacks**: go, python, node
...
## Configuration
The task uses configuration from `config/scheduled-tasks.yaml`:
