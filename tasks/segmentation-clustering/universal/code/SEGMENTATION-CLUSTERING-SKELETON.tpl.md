<!--
File: SEGMENTATION-CLUSTERING-SKELETON.tpl.md
Purpose: Template for segmentation-clustering implementation
Template Version: 1.0
-->

# SegmentationClustering Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for segmentation-clustering service
interface SegmentationClusteringService {
...
- `{{TASK_NAME}}`: Task name (segmentation-clustering)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/segmentation-clustering.yaml`:
