<!--
File: SEO-KEYWORD-RESEARCH-SKELETON.tpl.md
Purpose: Template for seo-keyword-research implementation
Template Version: 1.0
-->

# SeoKeywordResearch Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for seo-keyword-research service
interface SeoKeywordResearchService {
...
- `{{TASK_NAME}}`: Task name (seo-keyword-research)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/seo-keyword-research.yaml`:
