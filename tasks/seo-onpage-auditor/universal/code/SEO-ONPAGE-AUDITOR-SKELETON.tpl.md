<!--
File: SEO-ONPAGE-AUDITOR-SKELETON.tpl.md
Purpose: Template for seo-onpage-auditor implementation
Template Version: 1.0
-->

# SeoOnpageAuditor Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for seo-onpage-auditor service
interface SeoOnpageAuditorService {
...
- `{{TASK_NAME}}`: Task name (seo-onpage-auditor)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/seo-onpage-auditor.yaml`:
