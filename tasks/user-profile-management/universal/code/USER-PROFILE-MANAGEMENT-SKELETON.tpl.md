<!--
File: USER-PROFILE-MANAGEMENT-SKELETON.tpl.md
Purpose: Template for user-profile-management implementation
Template Version: 1.0
-->

# UserProfileManagement Service Skeleton
...
## Interface
```{{EXTENSION}}
// Universal interface for user-profile-management service
...
- `{{TASK_NAME}}`: Task name (user-profile-management)
- `{{STACK}}`: Target stack
- `{{TIER}}`: Target tier
- `{{EXTENSION}}`: File extension for target stack

## Configuration
The service expects configuration in `config/user-profile-management.yaml`:
