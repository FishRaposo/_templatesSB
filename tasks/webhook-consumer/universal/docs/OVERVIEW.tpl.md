...
# Webhook Consumer Overview

## Purpose
Robust handling of third-party webhooks (Stripe, GitHub, etc.), idempotency, logging.

## Features
- **Core Functionality**: Robust handling of third-party webhooks (Stripe, GitHub, etc.), idempotency, logging.
...
const result = await webhookconsumerService.execute(input);
```
...
**All Supported Stacks**: node, python, go

## Tier Recommendations
- **basic-webhooks**: core
- **enterprise-webhooks**: full
...
## Configuration
The task uses configuration from `config/webhook-consumer.yaml`:
